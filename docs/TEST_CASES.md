...
|:---:|---|---|---|---|:---:|:---:|
| **TC-01** | Danh mục phòng (Room Catalog) | 1. Mở trang `/rooms`.<br>2. Chọn bộ lọc Cơ sở: "Kapi Stay Đà Nẵng - Biển Mỹ Khê" (Property ID: `22222222-2222-2222-2222-222222222222`).<br>3. Chọn bộ lọc Số khách: 4 khách.<br>4. Quan sát danh sách phòng hiển thị. | • Cơ sở: `22222222-2222-2222-2222-222222222222`<br>• Số khách: 4 | Danh sách trả về đúng phòng 'Kapi Coastal Family Suite - Sơn Trà' (room_id: `b2222222-2222-2222-2222-222222222222`, capacity = 4) có sẵn trong `supabase/seed.sql` trên `main` thỏa mãn đồng thời cả 2 tiêu chí; các phòng khác cơ sở hoặc không đủ sức chứa bị loại trừ. | TV2 - Thảo | Chờ test |
...
| **TC-03** | Kiểm tra phòng trống (Availability Check) | 1. Chọn phòng `Kapi Deluxe Studio - Ban Công Phố Cổ` (cơ sở Hà Nội).<br>2. Chọn khoảng ngày lưu trú trùng với đơn đặt phòng mẫu trong database (từ `current_date` đến `current_date + 1`).<br>3. Quan sát phản hồi của hệ thống và nút CTA đặt phòng. | • Phòng: `Kapi Deluxe Studio - Ban Công Phố Cổ` (`a1111111-1111-1111-1111-111111111111`)<br>• Ngày: `current_date` đến `current_date + 1` (trùng mock booking `confirmed`) | Hàm `checkRoomAvailability()` trả về boolean `false` đối với khoảng ngày trùng lịch; UI RoomBookingWidget tiếp nhận và chuyển sang trạng thái `UNAVAILABLE`, hiển thị thông báo phòng đã kín và vô hiệu hóa nút Đặt phòng (CTA). | TV3 (Mai) & TV8 (Quỳnh) | Chờ test |
| **TC-04A** | Thanh toán VietQR — Render mã QR (Booking & Checkout) | 1. Đi tới màn hình `/checkout` với session ID `00000000-0000-0000-0000-000000000051` (trạng thái `ACTIVE`).<br>2. Chọn phương thức thanh toán VietQR.<br>3. Kiểm tra mã QR code và thông tin thanh toán. | • Session ID: `00000000-0000-0000-0000-000000000051`<br>• Trạng thái session: `ACTIVE`<br>• Phòng: `a1111111-1111-1111-1111-111111111111`<br>• Tổng tiền: 1.500.000 VND (2 đêm $\times$ 750k)<br>• Voucher: `99999999-9999-9999-9999-999999999999` (-400.000đ)<br>• Thực trả: 1.100.000 VND<br>• Nội dung CK: `KAPI51PAY` | Render mã VietQR chuẩn xác theo thông tin tài khoản, đúng số tiền thực trả 1.100.000đ (derive từ 1.500.000đ trừ 400.000đ voucher) và nội dung chuyển khoản chứa mã tham chiếu `KAPI51PAY` cho phiên checkout `ACTIVE` còn hiệu lực. UI KHÔNG được tự ý chuyển booking sang trạng thái thanh toán thành công khi chưa có xác nhận từ ngân hàng/hệ thống. | TV4 - Linh | Chờ test |
| **TC-04B** | Xác thực thanh toán & Chốt đơn atomic (Verified Finalization) | 1. Nhận xác nhận giao dịch ngân hàng thành công khớp chính xác `payment_reference = 'KAPI51PAY'` và số tiền 1.100.000đ.<br>2. Internal backend / payment-verifier gọi trusted RPC `finalize_verified_checkout_atomic(UUID, BIGINT, TEXT)` để chuyển trạng thái session và tạo booking atomic. | • Reference: `KAPI51PAY`<br>• Verified Amount: 1.100.000 VND | [PENDING / BLOCKED cho tới khi có payment verifier / webhook integration thật]. Checkout chỉ được finalize qua internal backend / payment-verifier execution path bằng `service_role` gọi hàm `finalize_verified_checkout_atomic(UUID, BIGINT, TEXT)`; browser/authenticated client KHÔNG được phép gọi trực tiếp; chuyển session sang `COMPLETED`, tạo đơn bookings (`booking_status = 'confirmed'`, `payment_status = 'paid'`), voucher sang `USED`; tuyệt đối không giả lập payment success bằng thao tác của người dùng trên UI. | TV4 (Linh) & TV8 (Quỳnh) | PENDING / BLOCKED |
| **TC-05** | Xác thực trước đặt phòng (Auth Guard) | 1. Mở cửa sổ ẩn danh (chưa đăng nhập).<br>2. Chọn phòng và nhấn nút "Đặt phòng ngay" từ Room Detail hoặc truy cập trực tiếp `/checkout`.<br>3. Quan sát luồng điều hướng của hệ thống. | • Trạng thái: Khách vãng lai (Chưa đăng nhập)<br>• Thao tác: Bấm đặt phòng hoặc truy cập checkout | Khách chưa đăng nhập bấm Đặt phòng từ Room Detail -> hệ thống bảo toàn các tham số đặt phòng (`roomId`, `checkIn`, `checkOut`, `guests`) và chuyển hướng đến `/login?next=<encoded checkout URL>`. Khi truy cập trực tiếp `/checkout` thì chuyển hướng đến `/login?next=/checkout`. | TV4 (Linh) & TV8 (Quỳnh) | Chờ test |
| **TC-06** | Trải nghiệm lưu trú (My Stay Concierge) | 1. Đăng nhập tài khoản có booking đang hiệu lực.<br>2. Vào trang `/my-stay`.<br>3. Kiểm tra thông tin mã cửa (Digital Key).<br>4. Nhấn nút "Sao chép mật khẩu Wi-Fi" và dán thử. | • **Precondition:** Thời điểm thực thi kiểm thử (`now()`) phải nằm trong khung giờ lưu trú hợp lệ (từ 14:00 ngày check-in đến 12:00 ngày check-out) hoặc sử dụng fixture môi trường khớp khung giờ test.<br>• Booking: Trạng thái `confirmed` (`00000000-0000-0000-0000-000000000021`) tại phòng `a1111111-1111-1111-1111-111111111111`<br>• Wi-Fi SSID: `KapiStay_HaNoi_5G`<br>• Pass: `kapistay2026` | Hệ thống hiển thị mã số mở cửa (Digital Key PIN) tương ứng với credential còn hiệu lực (`status = 'active'`); bấm nút copy Wi-Fi hiển thị thông báo "Đã sao chép" và clipboard lưu đúng mật khẩu Wi-Fi `kapistay2026`. | TV5 - Chi | Chờ test |
| **TC-07** | Báo sự cố & Tickets (Guest Tickets) | 1. Tại `/my-stay` với đơn booking hợp lệ, mở form "Báo sự cố thiết bị".<br>2. Chọn danh mục "Thiết bị hỏng", nhập mô tả lỗi.<br>3. Bấm "Gửi yêu cầu". | • Booking: Trạng thái `confirmed`<br>• Category: "Thiết bị hỏng" (thuộc TV6 Whitelist)<br>• Description: "Điều hòa không mát"<br>• Media upload: [PENDING / TBD — Media upload chưa có trusted upload/persistence flow, hiện dùng `media_paths = []`] | Gửi dữ liệu bất đồng bộ không reload trang (No Page Reload); chỉ hiển thị thông báo thành công khi ticket thực sự được ghi nhận và lưu thành công vào database với `status = 'pending'`; danh sách ticket tự cập nhật ticket mới.<br>[PENDING / TBD — Media upload chưa có trusted upload/persistence flow; flow kiểm thử hiện tại không đính kèm file ảnh]. | TV6 - Phương | Chờ test |
| **TC-08** | Vận hành & Quản trị (Operations Dashboard) | 1. Khách lưu trú gửi ticket sự cố từ trang `/my-stay` (TC-07).<br>2. Mở Operations Dashboard tại `/operations` và tải lại trang hoặc refetch danh sách ticket. | • Sự kiện: Ticket mới vừa tạo từ khách lưu trú | Operations Dashboard tải dữ liệu thành công, hiển thị đúng ticket vừa tạo khi reload/refetch. | TV7 - T Mai | Chờ test |
