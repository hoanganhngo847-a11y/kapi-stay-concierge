...
| **TC-04A** | Thanh toán VietQR — Render mã QR (Booking & Checkout) | 1. Đi tới màn hình `/checkout` với session ID `00000000-0000-0000-0000-000000000051` (trạng thái `ACTIVE`).<br>2. Chọn phương thức thanh toán VietQR.<br>3. Kiểm tra mã QR code và thông tin thanh toán. | • Session ID: `00000000-0000-0000-0000-000000000051`<br>• Trạng thái session: `ACTIVE`<br>• Tổng tiền: 1.300.000 VND (2 đêm $\times$ 650k)<br>• Voucher: `KAPI40` (-400.000đ)<br>• Thực trả: 900.000 VND<br>• Nội dung CK: `KAPI51PAY` | Render mã VietQR chuẩn xác theo thông tin tài khoản, đúng số tiền thực trả 900.000đ (derive từ 1.300.000đ trừ 400.000đ voucher) và nội dung chuyển khoản chứa mã tham chiếu `KAPI51PAY` cho phiên checkout `ACTIVE` còn hiệu lực. UI KHÔNG được tự ý chuyển booking sang trạng thái thanh toán thành công khi chưa có xác nhận từ ngân hàng/hệ thống. | TV4 - Linh | Chờ test |
| **TC-04B** | Xác thực thanh toán & Chốt đơn atomic (Verified Finalization) | 1. Nhận xác nhận giao dịch ngân hàng thành công khớp chính xác `payment_reference = 'KAPI51PAY'` và số tiền 900.000đ.<br>2. Backend gọi trusted RPC (`finalize_checkout_session`) để chuyển trạng thái session và tạo booking atomic. | • Reference: `KAPI51PAY`<br>• Verified Amount: 900.000 VND | [PENDING / BLOCKED cho tới khi có payment verifier / webhook integration thật]. Checkout chỉ được finalize qua trusted backend sau khi xác minh giao dịch thực tế; chuyển session sang `COMPLETED`, tạo đơn bookings (`booking_status = 'confirmed'`, `payment_status = 'paid'`), voucher sang `USED`; tuyệt đối không giả lập payment success bằng thao tác của người dùng trên UI. | TV4 (Linh) & TV8 (Quỳnh) | PENDING / BLOCKED |
...
| **TC-06** | Trải nghiệm lưu trú (My Stay Concierge) | 1. Đăng nhập tài khoản có booking đang hiệu lực.<br>2. Vào trang `/my-stay`.<br>3. Kiểm tra thông tin mã cửa (Digital Key).<br>4. Nhấn nút "Sao chép mật khẩu Wi-Fi" và dán thử. | • **Precondition:** Thời điểm thực thi kiểm thử (`now()`) phải nằm trong khung giờ lưu trú hợp lệ (từ 14:00 ngày check-in đến 12:00 ngày check-out) hoặc sử dụng fixture môi trường khớp khung giờ test.<br>• Booking: Trạng thái `confirmed` (`00000000-0000-0000-0000-000000000021`)<br>• Wi-Fi SSID: `Kapi_DangVanNgu_5G`<br>• Pass: `kapi@dvn2026` | Hệ thống hiển thị mã số mở cửa (Digital Key PIN) tương ứng với credential còn hiệu lực (`status = 'active'`); bấm nút copy Wi-Fi hiển thị thông báo "Đã sao chép" và clipboard lưu đúng mật khẩu Wi-Fi. | TV5 - Chi | Chờ test |
| **TC-07** | Báo sự cố & Tickets (Guest Tickets) | 1. Tại `/my-stay` với đơn booking hợp lệ, mở form "Báo sự cố thiết bị".<br>2. Chọn danh mục "Thiết bị điện lạnh", nhập mô tả lỗi.<br>3. Bấm "Gửi yêu cầu". | • Booking: Trạng thái `confirmed`<br>• Category: "Thiết bị điện lạnh"<br>• Description: "Điều hòa không mát"<br>• Media upload: [PENDING / TBD — Media upload chưa có trusted upload/persistence flow, hiện dùng `media_paths = []`] | Gửi dữ liệu bất đồng bộ không reload trang (No Page Reload); chỉ hiển thị thông báo thành công khi ticket thực sự được ghi nhận và lưu thành công vào database với `status = 'pending'`; danh sách ticket tự cập nhật ticket mới.<br>[PENDING / TBD — Media upload chưa có trusted upload/persistence flow; flow kiểm thử hiện tại không đính kèm file ảnh]. | TV6 - Phương | Chờ test |
